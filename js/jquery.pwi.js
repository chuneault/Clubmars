...
(function ($) {
    ...
    $.fn.pwi = function (opts) {
        ...
        function alignPictures(divName) {
            ...
            $(divName).css('height', (divHeigth)+'px');
            if (settings.thumbAlign == 1) {
                $(divName).css('width', (divWidth)+'px');
            }
        }
        ...
        function photo(j, hidden, username) {
            ...
            else
            {
                ...
                if ($youtubeId == "") {
					...
					var newheight = j.media$group.media$thumbnail[0].height;
					var newwidth = j.media$group.media$thumbnail[0].width;


					if (newheight < settings.thumbSize) {

					   var scalexy = settings.thumbSize - newheight;
					   newheight += scalexy;
					   newwidth += scalexy;
...
;
			...
			if (opts.after_load_album) { opts.after_load_album(); }
        ...
        function getAlbum() {
            if (settings.photostore[settings.album]) {
                ...
                show(true, '');
                var test = $.getJSON($u, 'callback=?', album);
